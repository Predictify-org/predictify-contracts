...
// ===== MARKET TTL BUMP TESTS =====

#[test]
fn test_ttl_constants_are_sane() {
    /// Verifies that TTL constants follow sensible relationships.
    /// The threshold must be less than the bump amount (otherwise the bump never fires),
    /// and the bump amount should be at least 1 day of ledgers.
    assert!(
        MARKETS_LIFETIME_THRESHOLD < MARKETS_BUMP_AMOUNT,
        "threshold must be less than bump amount"
    );
    assert!(
        MARKETS_BUMP_AMOUNT >= 17_280,
        "bump amount should be at least 1 day of ledgers"
    );
}

#[test]
fn test_market_ttl_extended_on_create() {
    /// Verifies that creating a market bumps its storage TTL.
    /// The market should be readable after creation without panicking on TTL extension.
    let test = PredictifyTest::setup();
    let client = PredictifyHybridClient::new(&test.env, &test.contract_id);
    ...
    let market_id = client.create_market(
        &test.admin,
        &String::from_str(&test.env, "TTL Create Test"),
        &outcomes,
        &30,
        &OracleConfig {
            provider: OracleProvider::reflector(),
    ...
    // Verify the market is stored and readable
    let market = test.env.as_contract(&test.contract_id, || {
        test.env
            .storage()
            .persistent()
    ...
    assert!(market.is_some(), "Market should be stored after creation");
    let m = market.unwrap();
    assert_eq!(m.question, String::from_str(&test.env, "TTL Create Test"));
}

#[test]
fn test_market_ttl_extended_on_read() {
    /// Verifies that reading a market bumps its storage TTL.
    /// Creating and then reading a market should both extend TTL without panic.
    let test = PredictifyTest::setup();
    let client = PredictifyHybridClient::new(&test.env, &test.contract_id);
    ...
    let market_id = client.create_market(
        &test.admin,
        &String::from_str(&test.env, "TTL Read Test"),
        &outcomes,
        &30,
        &OracleConfig {
            provider: OracleProvider::reflector(),
    ...
    // Call get_market via contract environment (simulates read hot path)
    test.env.as_contract(&test.contract_id, || {
        let market = crate::markets::MarketStateManager::get_market(&test.env, &market_id);
        assert!(market.is_ok(), "Market should be readable");
        let m = market.unwrap();
        assert_eq!(m.question, String::from_str(&test.env, "TTL Read Test"));
    });
}

#[test]
fn test_market_ttl_extended_on_update() {
    /// Verifies that updating a market bumps its storage TTL.
    /// Creating, then adding a vote (which updates market state), should both extend TTL.
    let test = PredictifyTest::setup();
    let client = PredictifyHybridClient::new(&test.env, &test.contract_id);
    ...
    let market_id = client.create_market(
        &test.admin,
        &String::from_str(&test.env, "TTL Update Test"),
        &outcomes,
        &30,
        &OracleConfig {
            provider: OracleProvider::reflector(),
    ...
    // Place a vote to trigger market update
    let voter = test.create_funded_user();
    test.env.mock_all_auths();
    client.vote(
        &voter,
    ...
    // Verify the market was updated with the vote
    let market = test.env.as_contract(&test.contract_id, || {
        test.env
            .storage()
            .persistent()
    ...
    assert!(market.is_some(), "Market should exist after vote");
    let m = market.unwrap();
    assert_eq!(m.total_staked, 100_0000000, "Vote should be recorded");
}

#[test]
fn test_bump_does_not_panic_on_valid_market_key() {
    /// Verifies that the bump_market_ttl helper doesn't panic on valid market keys.
    /// This is a sanity check that the extend_ttl call is safe to use on hot paths.
    let test = PredictifyTest::setup();
    let client = PredictifyHybridClient::new(&test.env, &test.contract_id);
    ...
    let market_id = client.create_market(
        &test.admin,
        &String::from_str(&test.env, "Bump Safety Test"),
        &outcomes,
        &30,
        &OracleConfig {
            provider: OracleProvider::reflector(),
            oracle_address: Address::generate(&test.env),
            feed_id: String::from_str(&test.env, "ADA"),
            threshold: 10,
            comparison: String::from_str(&test.env, "gt"),
        },
        &None,
    ...
    // Call bump_market_ttl directly on the valid market key
    test.env.as_contract(&test.contract_id, || {
        // This should not panic
        crate::storage::bump_market_ttl(&test.env, &market_id);
    });

    // Verify market is still accessible after the bump
    let market = test.env.as_contract(&test.contract_id, || {
        test.env
            .storage()
            .persistent()
    ...
    assert!(
        market.is_some(),
        "Market should still be accessible after TTL bump"
    );
}

#[test]
fn test_multiple_sequential_bumps_safe() {
    /// Verifies that calling bump_market_ttl multiple times is safe and doesn't cause issues.
    /// extend_ttl is idempotent and only extends TTL, never shortens it.
    let test = PredictifyTest::setup();
    let client = PredictifyHybridClient::new(&test.env, &test.contract_id);
    ...
    let market_id = client.create_market(
        &test.admin,
        &String::from_str(&test.env, "Sequential Bump Test"),
        &outcomes,
        &30,
        &OracleConfig {
            provider: OracleProvider::reflector(),
            oracle_address: Address::generate(&test.env),
            feed_id: String::from_str(&test.env, "DOT"),
            threshold: 75,
            comparison: String::from_str(&test.env, "gt"),
        },
        &None,
    ...
    // Bump the market TTL multiple times in quick succession
    test.env.as_contract(&test.contract_id, || {
        for _ in 0..5 {
            crate::storage::bump_market_ttl(&test.env, &market_id);
        }
    ...
    // Market should still be readable and valid
    let market = test.env.as_contract(&test.contract_id, || {
        test.env
            .storage()
            .persistent()
    ...
    assert!(market.is_some(), "Market should survive multiple sequential bumps");
}
