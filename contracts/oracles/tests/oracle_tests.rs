...
fn test_oracle_lifecycle_and_queries() {
    ...
    client.add_oracle(&admin, &price_only_addr);
    ...
    assert_eq!(client.get_price(&price_only_addr, &feed), 54321);

    // Query price data from price-only oracle (should fallback)
    let fallback_pdata = client.get_price_data(&price_only_addr, &feed);
    ...
    client.add_oracle(&admin, &failing_addr);

    // Failing oracle is not healthy
    assert!(!client.is_oracle_healthy(&failing_addr));

    // Failing oracle get_price returns error
    let price_res = client.try_get_price(&failing_addr, &feed);
    assert_eq!(price_res, Err(Ok(Error::OracleUnavailable)));

    // Failing oracle get_price_data returns error
    let pdata_res = client.try_get_price_data(&failing_addr, &feed);
    assert_eq!(pdata_res, Err(Ok(Error::OracleUnavailable)));

    // 8. Query unregistered oracle
    let unregistered_addr = Address::generate(&env);
    assert_eq!(
        client.try_get_price(&unregistered_addr, &feed),
        Err(Ok(Error::InvalidOracleConfig))
    );
    assert_eq!(
        client.try_get_price_data(&unregistered_addr, &feed),
        Err(Ok(Error::InvalidOracleConfig))
    );
    assert_eq!(
