...
use hex;
    /// Set the multisig threshold for admin actions (M-of-N)
    ///
    /// Only callable by an authorized admin. Emits event on change.
    pub fn set_multisig_threshold(env: Env, admin: Address, threshold: u32) -> Result<(), Error> {
        admin.require_auth();
        crate::admin::MultisigManager::set_threshold(&env, &admin, threshold)
    }
