...
#[test]
#[should_panic]
fn test_upgrade_contract_requires_auth() {
    let (env, _admin, contract_id) = setup_test_env();

    env.as_contract(&contract_id, || {
        let new_wasm_hash = BytesN::from_array(&env, &[1u8; 32]);
        let non_admin = Address::generate(&env);
        UpgradeManager::upgrade_contract(&env, &non_admin, new_wasm_hash).unwrap();
    });
}

#[test]
fn test_upgrade_contract_with_auth() {
    let (env, admin, contract_id) = setup_test_env();
    ...
    env.as_contract(&contract_id, || {
        ...
        let result = UpgradeManager::upgrade_contract(&env, &admin, new_wasm_hash);
        assert!(result.is_ok());
    });
}
...
#[test]
#[should_panic]
fn test_rollback_upgrade_requires_auth() {
    let (env, _admin, contract_id) = setup_test_env();

    env.as_contract(&contract_id, || {
        let rollback_wasm_hash = BytesN::from_array(&env, &[2u8; 32]);
        let non_admin = Address::generate(&env);
        UpgradeManager::rollback_upgrade(&env, &non_admin, rollback_wasm_hash).unwrap();
    });
}

#[test]
fn test_rollback_upgrade_with_auth() {
    let (env, admin, contract_id) = setup_test_env();
    ...
    env.as_contract(&contract_id, || {
        let rollback_wasm_hash = BytesN::from_array(&env, &[2u8; 32]);
        let result = UpgradeManager::rollback_upgrade(&env, &admin, rollback_wasm_hash);
        assert!(result.is_ok());
    });
}
