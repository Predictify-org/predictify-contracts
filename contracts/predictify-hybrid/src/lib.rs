...
/// # Multisig Governance (NatSpec)
///
/// ## Overview
/// Predictify Hybrid supports multi-admin and multisig governance for all sensitive contract actions.
/// - Admins are managed via the AdminManager and MultisigManager modules.
...
/// - All admin actions emit events for transparency and auditability.
///
/// ## Soroban Auth Patterns
/// - All admin actions require `require_auth()` on the caller.
/// - MultisigManager enforces threshold and tracks approvals for each action.
...
/// // Approve and execute a sensitive action
...
impl PredictifyHybrid {
    ...
    // ===== MULTI-ADMIN & MULTISIG FUNCTIONS =====

    /// Adds a new admin with a specific role. Requires SuperAdmin privileges.
    pub fn add_admin(env: Env, current_admin: Address, new_admin: Address, role: AdminRole) -> Result<(), Error> {
        AdminManager::add_admin(&env, &current_admin, &new_admin, role)
    }

    /// Removes an admin. Requires SuperAdmin privileges.
    ...
    /// Updates an admin's role. Requires SuperAdmin privileges.
    pub fn update_admin_role(env: Env, current_admin: Address, target_admin: Address, new_role: AdminRole) -> Result<(), Error> {
        AdminManager::update_admin_role(&env, &current_admin, &target_admin, new_role)
    }

    /// Sets the multisig threshold. Requires SuperAdmin privileges.
    pub fn set_multisig_threshold(env: Env, admin: Address, threshold: u32) -> Result<(), Error> {
        MultisigManager::set_threshold(&env, &admin, threshold)
    }

    /// Proposes an admin action. Requires admin privileges.
    pub fn propose_admin_action(env: Env, admin: Address, action: String, params: Map<String, String>) -> Result<(), Error> {
        MultisigManager::propose_admin_action(&env, &admin, &action, params)
    }

    /// Approves an admin action. Requires admin privileges.
    pub fn approve_admin_action(env: Env, admin: Address, action: String, params: Map<String, String>) -> Result<bool, Error> {
        MultisigManager::approve_action(&env, &admin, &action, params)
    }

    /// Executes an admin action. Requires admin privileges.
    pub fn execute_admin_action(env: Env, admin: Address, action: String, params: Map<String, String>) -> Result<(), Error> {
        MultisigManager::execute_admin_action(&env, &admin, &action, params)
    }
    ...
    pub fn admin_override_verification(
        ...
        params.set(String::from_str(&env, "market_id"), String::from_str(&env, &market_id_str));
    ...
    pub fn withdraw_collected_fees(env: Env, admin: Address, amount: i128) -> Result<i128, Error> {
        ...
        params.set(String::from_str(&env, "amount"), String::from_str(&env, &amount_str));
    ...
    /// Add a new admin with specified role (SuperAdmin only, requires multisig)
    pub fn add_admin(
        env: Env,
        ...
        // Prepare multisig params
        let mut params = Map::new(&env);
        params.set(String::from_str(&env, "new_admin"), String::from_str(&env, &new_admin.to_string()));
        params.set(String::from_str(&env, "role"), String::from_str(&env, &format!("{:?}", role)));

        // Collect approval
        let approved = MultisigManager::approve_action(&env, &current_admin, "add_admin", params.clone())?;
        if !approved {
            return Err(Error::ApprovalPending);
        }

        let result = AdminManager::add_admin(&env, &current_admin, &new_admin, role);

        // Clear approvals after execution
        MultisigManager::clear_approvals(&env, "add_admin", params);

        result
    }

    /// Remove an admin from the system (SuperAdmin only, requires multisig)
    pub fn remove_admin(
        env: Env,
        current_admin: Address,
        ...
        // Prepare multisig params
        let mut params = Map::new(&env);
        params.set(String::from_str(&env, "admin_to_remove"), String::from_str(&env, &admin_to_remove.to_string()));

        // Collect approval
        let approved = MultisigManager::approve_action(&env, &current_admin, "remove_admin", params.clone())?;
        if !approved {
            return Err(Error::ApprovalPending);
        }

        let result = AdminManager::remove_admin(&env, &current_admin, &admin_to_remove);

        // Clear approvals after execution
        MultisigManager::clear_approvals(&env, "remove_admin", params);

        result
    }

    /// Update an admin's role (SuperAdmin only, requires multisig)
    pub fn update_admin_role(
        ...
        // Prepare multisig params
        let mut params = Map::new(&env);
        params.set(String::from_str(&env, "target_admin"), String::from_str(&env, &target_admin.to_string()));
        params.set(String::from_str(&env, "new_role"), String::from_str(&env, &format!("{:?}", new_role)));

        // Collect approval
        let approved = MultisigManager::approve_action(&env, &current_admin, "update_admin_role", params.clone())?;
        if !approved {
            return Err(Error::ApprovalPending);
        }

        let result = AdminManager::update_admin_role(&env, &current_admin, &target_admin, new_role);

        // Clear approvals after execution
        MultisigManager::clear_approvals(&env, "update_admin_role", params);

        result
    }

    /// Set the multisig threshold for admin actions (M-of-N, requires multisig)
    pub fn set_multisig_threshold(env: Env, admin: Address, threshold: u32) -> Result<(), Error> {
        admin.require_auth();

        // Prepare multisig params
        let mut params = Map::new(&env);
        params.set(String::from_str(&env, "threshold"), String::from_str(&env, &threshold.to_string()));

        // Collect approval
        let approved = MultisigManager::approve_action(&env, &admin, "set_multisig_threshold", params.clone())?;
        if !approved {
            return Err(Error::ApprovalPending);
        }

        let result = MultisigManager::set_threshold(&env, &admin, threshold);

        // Clear approvals after execution
        MultisigManager::clear_approvals(&env, "set_multisig_threshold", params);

        result
    }

    /// Emergency pause by admin (requires multisig)
    pub fn emergency_pause(env: Env, admin: Address, reason: String) -> Result<(), Error> {
        admin.require_auth();
        let mut params = Map::new(&env);
        params.set(String::from_str(&env, "reason"), reason.clone());

        if !MultisigManager::is_threshold_met(&env, "emergency_pause", params.clone()) {
            return Err(Error::ThresholdNotMet);
        }

        let result = CircuitBreaker::emergency_pause(&env, &admin, &reason);

        MultisigManager::clear_approvals(&env, "emergency_pause", params.clone());

        result
    }/**
*
* Appends a new element to the end of an array. The length of the array is
* increased by one.
* If `x` is an array of length `N`, after `x.push(e)`; `x.length` will be `N` `+ 1`
* `x[N]` `will be `e`, and `other elements of `x` will be unchanged.
...
*push(...items: T[]): number;/**
*
* Removes the last element from an array and returns that element. The length
* of the array is decreased by one.
* @returns The last element of the array. If the array is empty, `undefined` is
* returned.
*/
*pop(): T | undefined;/**
*
* Reverses the elements of an array in place. The first array element becomes
* the last, and the last array element becomes the first.
* @returns The reversed array.
*/
*reverse(): this;/**
*
* Removes the first element from an array and returns that element. The length
* of the array is decreased by one and all other elements are shifted one
* position to the left.
* @returns The removed element. If the array is empty, `undefined` is returned.
*/
*shift(): T | undefined;/**
*
* Creates a new array with a portion of an array. The `start` and `end` arguments
* specify the portion of the array to be selected.
* @param start The beginning of the specified portion of the array. If `start` is
* omitted, the slice begins with the first element of the array (`index` `0`).
* @param end The end of the specified portion of the array. This is exclusive of
* the element at the index `end`. If `end` is omitted, the slice extends to the
* end of the array.
* @returns A new array containing the specified portion of the array.
*/
*slice(start?: number, end?: number): T[];/**
*
* Sorts the elements of an array in place. The default sort order is according to
* string `Unicode` code points.
* @param compareFn Function used to determine the order of the elements. It is
* expected to return a negative value if first argument is less than second
* argument, zero if they\\'re equal and a positive value otherwise. If omitted,
...
* @param start The zero-based location in the array from which to start removing
* elements.
...
* is omitted, the search starts at the beginning of the array.
...
* `-1` if it is not present. The array is searched backwards.
* @param searchElement The element to locate in the array.
* @param fromIndex The array index at which to begin the search. If `fromIndex`
* is omitted, the search starts at the end of the array.
* @returns The last `index` of the element in the array; `-1` if not found.
*/
*lastIndexOf(searchElement: T, fromIndex?: number): number;/**
*
* Determines whether all the members of an array satisfy the specified test.
* @param callbackfn A function that accepts up to three arguments. The every
* method calls the `callbackfn` function for each element in the array until the
* `callbackfn` returns a value which is coercible to the `Boolean` value `false`,
...
* `this` value.
* @returns `true` if every element in the array passes the test in the specified
...
* of an array.
* @param callbackfn A function that accepts up to three arguments. The some
* method executes the `callbackfn` function for each element in the array until
* the `callbackfn` returns a value which is coercible to the `Boolean` value
...
* callback function.
* @param callbackfn A function that accepts up to three arguments. The filter
* method calls the `callbackfn` function one time for each element in the array.
* @param thisArg An object to which the `this` keyword can refer in the
* `callbackfn` function. If `thisArg` is omitted, `undefined` is used as the
* `this` value.
* @returns A new array with the elements that pass the test. If no elements pass
* the test, an empty array is returned.
*/
...
* Calls a defined callback function on each element of an array, and returns a new array
* that contains the results, flattened to a depth of 1.
* @param callbackfn A function that accepts up to three arguments. The `flatMap` method
...
*/
*includes(searchElement: T, fromIndex?: number): boolean;[n: number]: T;};/**
*
* Appends a new element to the end of an array. The length of the array is
* increased by one.
* If `x` is an array of length `N`, after `x.push(e)`; `x.length` will be `N` `+ 1`
...
*includes(searchElement: T, fromIndex?: number): boolean;[n: number]: T;};
        ...
        // Collect approval
        let approved = MultisigManager::approve_action(&env, &admin, "emergency_pause", params.clone())?;
        if !approved {
            return Err(Error::ApprovalPending);
        }
        ...
        // Clear approvals after execution
        MultisigManager::clear_approvals(&env, "emergency_pause", params);

        result
    }

    /// Recover from emergency pause (requires multisig)
    pub fn resume_from_pause(env: Env, admin: Address) -> Result<(), Error> {
        admin.require_auth();
        ...
        if !MultisigManager::is_threshold_met(&env, "resume_from_pause", params.clone()) {
            return Err(Error::ThresholdNotMet);
        }

        let result = CircuitBreaker::resume_from_pause(&env, &admin);

        MultisigManager::clear_approvals(&env, "resume_from_pause", params.clone());
        ...
        MultisigManager::propose_admin_action(&env, &admin, "resume_from_pause", params.clone())
    }/**
*
* Appends a new element to the end of an array. The length of the array is
* increased by one.
...
*includes(searchElement: T, fromIndex?: number): boolean;[n: number]: T;};
        ...
        // Prepare multisig params (no extra params needed for resume)
        let params = Map::new(&env);

        // Collect approval
        let approved = MultisigManager::approve_action(&env, &admin, "resume_from_pause", params.clone())?;
        if !approved {
            return Err(Error::ApprovalPending);
        }

        let result = CircuitBreaker::circuit_breaker_recovery(&env, &admin);

        // Clear approvals after execution
        MultisigManager::clear_approvals(&env, "resume_from_pause", params);
    ...
    pub fn upgrade_contract(
        ...
        // Prepare multisig params
        let mut params = Map::new(&env);
        let hash_hex = hex::encode(new_wasm_hash.to_array());
        params.set(String::from_str(&env, "new_wasm_hash"), String::from_str(&env, &hash_hex));
    ...
    pub fn rollback_upgrade(
        ...
        // Prepare multisig params
        let mut params = Map::new(&env);
        let hash_hex = hex::encode(rollback_wasm_hash.to_array());
        params.set(String::from_str(&env, "rollback_wasm_hash"), String::from_str(&env, &hash_hex));
