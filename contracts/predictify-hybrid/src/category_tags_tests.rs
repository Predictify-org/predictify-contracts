...
fn test_query_events_by_category() {
    ...
    let (empty_events, _) =
        client.query_events_by_category(&String::from_str(&env, "None"), &0, &10);
...
fn test_query_events_by_tags() {
    ...
    // Set tags
    // m1: [L1, HighVol]
    client.update_event_tags(
        &admin,
        &m1,
        &vec![&env, tag_l1.clone(), tag_high_vol.clone()],
    );
    ...
    let (all_events, _) =
        client.query_events_by_tags(&vec![&env, tag_l1.clone(), tag_stable.clone()], &0, &10);
    assert_eq!(all_events.len(), 3);

    // Query by non-existing tag
    let (no_events, _) =
        client.query_events_by_tags(&vec![&env, String::from_str(&env, "NonExistent")], &0, &10);
...
fn test_security_update_tags_unauthorized() {
    ...
    client.update_event_tags(
        &user,
        &market_id,
        &vec![&env, String::from_str(&env, "Hack")],
    );
}
...
fn test_category_tags_do_not_affect_resolution_and_payouts() {
    ...
    client.update_event_category(
        &setup.admin,
        &setup.market_id,
        &Some(String::from_str(&setup.env, "Finance")),
    );
    client.update_event_tags(
        &setup.admin,
        &setup.market_id,
        &vec![&setup.env, String::from_str(&setup.env, "crypto")],
    );
...
fn test_update_category_after_bets_forbidden() {
    ...
    client.update_event_category(
        &setup.admin,
        &setup.market_id,
        &Some(String::from_str(&setup.env, "Blocked")),
    );
}
