...
use admin::{AdminAnalyticsResult, AdminInitializer, AdminManager, AdminPermission, AdminRole, MultisigConfig};
...
impl PredictifyHybrid {
    ...
    pub fn initialize(env: Env, admin: Address, platform_fee_percentage: Option<i128>) {
        ...
        // Initialize multisig with default threshold of 1 (single admin mode)
        // This allows seamless upgrade to multi-admin later
    ...
    /// 
    /// This sets how many admin approvals are required to execute sensitive operations
    /// like pausing the contract, upgrading, or withdrawing fees.
    /// 
    /// # Parameters
    /// * `env` - The Soroban environment
    /// * `admin` - The admin setting the threshold (must have EmergencyActions permission)
    /// * `threshold` - The number of required approvals (must be > 0 and <= number of admins)
    /// 
    /// # Example
    /// 
    /// ```rust
    ...
    /// 
    /// Returns the current threshold and total number of admins.
    pub fn get_multisig_config(env: Env) -> admin::MultisigConfig {
        use crate::admin::MultisigManager;
        MultisigManager::get_config(&env)
    }

    /// Propose a sensitive admin action that requires multisig approval
    /// 
    /// This is the first step in the multisig flow. The action is proposed but
    /// not executed until the threshold is met.
    /// 
    /// # Parameters
    /// * `env` - The Soroban environment
    /// * `admin` - The admin proposing the action
    /// * `action` - The action name (e.g., "pause", "upgrade_contract", "withdraw_collected_fees")
    ...
    /// Approve a sensitive admin action that requires multisig approval
    /// 
    /// This is the second step in the multisig flow. Each admin calls this to approve.
    /// Once the threshold is met, the action can be executed.
    /// 
    /// # Parameters
    /// * `env` - The Soroban environment
    /// * `admin` - The admin approving the action
    /// * `action` - The action name
    /// * `params` - Map of action parameters (must match the proposed action)
    /// 
    /// # Returns
    /// * `Ok(true)` - Threshold met, action can be executed
    /// * `Ok(false)` - Approval recorded, waiting for more approvals
    ...
    /// Execute an approved sensitive admin action
    /// 
    /// This is the final step in the multisig flow. It verifies that the threshold
    /// has been met and clears the approvals after execution.
    ...
    /// Emergency pause the contract (requires multisig approval)
    ///
    /// This function triggers an emergency pause of the contract, halting all
    /// market operations. It requires multisig approval based on the current threshold.
    ///
    /// # Parameters
    /// * `env` - The Soroban environment
    /// * `admin` - The admin initiating the pause
    /// * `reason` - The reason for the emergency pause
    pub fn emergency_pause(
        ...
        // Execute the pause
        let result = circuit_breaker::CircuitBreaker::emergency_pause(&env, &admin, &reason);
    ...
    /// Resume from emergency pause (requires multisig approval)
    ///
    /// This function recovers from an emergency pause state, resuming normal
    /// market operations. It requires multisig approval based on the current threshold.
    pub fn emergency_resume(
        ...
        // Collect approval
        let approved = MultisigManager::approve_action(&env, &admin, "emergency_resume", params.clone())?;
        if !approved {
            // Not enough approvals yet
            return Err(Error::ApprovalPending);
        }
        ...
        // Execute the resume
        let result = circuit_breaker::CircuitBreaker::circuit_breaker_recovery(&env, &admin);

        // Clear approvals after execution
        MultisigManager::clear_approvals(&env, "emergency_resume", params);
    ...
    pub fn get_circuit_breaker_status(env: Env) -> Result<Map<String, String>, Error> {
        circuit_breaker::CircuitBreaker::get_circuit_breaker_status(&env)
    }
