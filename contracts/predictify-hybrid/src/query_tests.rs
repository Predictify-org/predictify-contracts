...
fn test_payout_never_exceeds_total_pool() {
    ...
    let mut wins = Vec::new(&env);
    wins.push_back(String::from_str(&env, "yes"));
    market.winning_outcomes = Some(wins);
...
fn test_status_conversion_roundtrip() {
    // Test that we can convert states and back (use alloc::vec! for Rust Vec, not soroban_sdk::vec!)
    let all_states = alloc::vec![
        MarketState::Active,
        MarketState::Ended,
        MarketState::Disputed,
        MarketState::Resolved,
