...
fn test_payout_blocked_during_dispute_window() {
    ...
    setup.env.as_contract(&setup.contract_id, || {
        ...
        market
            .votes
            .set(user.clone(), String::from_str(&setup.env, "yes"));
        ...
        assert!(
            ...
            "Should still be in dispute window"
        );
...
fn test_payout_allowed_after_dispute_window_closes() {
    ...
    setup.env.as_contract(&setup.contract_id, || {
        ...
        assert!(
            current_time >= final_market.end_time,
            "Should be past dispute window"
        );
    });
}
...
fn test_dispute_creation_during_window() {
    ...
    setup.env.as_contract(&setup.contract_id, || {
        ...
        market.state = MarketState::Ended;
        market.winning_outcomes = Some(vec![&setup.env, String::from_str(&setup.env, "yes")]);
        setup.env.storage().persistent().set(&market_id, &market);
...
fn test_multiple_disputes_extend_once() {
    ...
    setup.env.as_contract(&setup.contract_id, || {
        ...
        market
            .dispute_stakes
            .set(user2.clone(), existing_stake + 10_000_000);
        ...
        assert_eq!(
            market.dispute_stakes.get(user1.clone()).unwrap(),
            10_000_000
        );
        assert_eq!(
            market.dispute_stakes.get(user2.clone()).unwrap(),
            10_000_000
        );
