...
use crate::types::{ReflectorAsset, OracleConfig, OracleProvider};
...
impl ReflectorAssetMatrixTest {
    ...
    pub fn test_property_consistency(&self) {
        self.test_all_supported(|asset| {
            ...
            assert!(asset.is_supported(), "All tested assets should be supported");
            assert!(asset.is_known(), "All tested assets should be known");
            
            // Validation consistency
            assert!(asset.validate_for_market(&self.env).is_ok(), 
    ...
    pub fn test_symbol_round_trip(&self) {
        self.test_all_supported(|asset| {
            ...
            let reconstructed = ReflectorAsset::from_symbol(String::from_str(&self.env, &symbol_str));
            assert_eq!(asset, &reconstructed, "Round-trip conversion should preserve asset");
        });
    }
    ...
    pub fn test_feed_id_format(&self) {
        self.test_all_supported(|asset| {
            ...
            assert!(!feed_id.starts_with("/"), "Feed ID should not start with '/'");
...
impl TokenRegistryTestUtils {
    ...
    pub fn verify_registry_contains(&self, expected_symbols: Vec<&str>) {
        ...
        for symbol in expected_symbols.iter() {
            let symbol_found = global_assets.iter().any(|asset| {
                asset.symbol.to_string() == *symbol
            });
            assert!(symbol_found, "Registry should contain asset with symbol: {}", symbol);
        }
    }
    ...
    pub fn test_asset_validation(&self) {
        ...
        assert!(crate::tokens::TokenRegistry::validate_asset(&self.env, &valid_asset, None).is_ok(),
                "Valid asset should pass registry validation");

        // Invalid asset should fail
        assert!(crate::tokens::TokenRegistry::validate_asset(&self.env, &invalid_asset, None).is_err(),
        ...
        self.env.as_contract(contract_id, || {
            ...
            for asset in assets.iter() {
                ...
                let market_id = PredictifyHybrid::create_market(
                    self.env.clone(),
                    admin.clone(),
                    String::from_str(&self.env, &format!("Test market for {}", asset.name().to_string())),
                    outcomes,
                    30,
                    oracle_config,
                    None,
