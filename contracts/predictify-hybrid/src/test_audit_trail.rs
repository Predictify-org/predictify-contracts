...
use soroban_sdk::{testutils::Address as _, Address, Env, Map, Symbol, String, BytesN};
...
fn test_append_and_get_record() {
    ...
    env.as_contract(&contract_id, || {
        ...
        assert_eq!(record1.prev_record_hash, BytesN::from_array(&env, &[0u8; 32]));
...
fn test_verify_integrity_tampering() {
    ...
    env.as_contract(&contract_id, || {
        ...
        env.storage().persistent().set(&(Symbol::new(&env, "AUDIT_REC"), 1u64), &record1);
